...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, long_or_short: str, enter_or_exit: str, price: float = 0.0, stop_limit_price: float = 0.0) -> dict:
        ...
        self.order = {
            ...
            "orderLegCollection": [
                {
                "instruction": self._order_instructions[enter_or_exit][long_or_short],
                    "quantity": 0,
                    "instrument":{
                        "symbol": None,
                        "assetType": None
        ...
        elif self.order['orderType'] == 'STOP LIMIT':
            self.order['stopPrice'] = price
        ...
        if order_type == 'stop':
            ...
            self.stop_price = 0.0
        #Store important side information
        ...
        def modify_side(self, side: Optional[str], order_leg_id: int = 0) -> None:
            if side and side not in ['buy', 'sell', 'sell_short', 'buy_to_cover']:
                raise ValueError("You passed through a invalid side")
            if side:
                self.side['orderLegCollection'][order_leg_id]['instructions'] = side.upper()
            else:
                self.order['orderLegCollection'][order_leg_id]['instructions'] = self._order_instructions[self.enter_or_exit][self.side_opposite]
        ...
        def add_stop_loss(self, stop_size: float, percentage: bool = False) -> bool:
            ...
            if percentage:
                ...
                new_price = self._calculate_new_price(priec=price, adjustment=adjustment, percentage=True)
            # Add the order
            stop_limit_order = {
        ...
        def add_stop_limit(self,stop_size: float, limit_size: float, stop_percentage: bool = False, limit_percentage: bool = False) -> bool:
            ...
            if stop_percentage:
                ...
                adjustment = -stop_size
                stop_price = self._calculate_new_price(priec=price, adjustment=adjustment, percentage=True)
            if limit_percentage:
            ...
            def _calculate_new_price(self, price: float, adjustment: float, percentage:bool) -> float:
                ...
                return new_price
            def add_take_profit(self, profit_loss: float, percentage: bool =False)-> bool:
                if not self._triggered_added:
                    self._convert_to_trigger()
                if not self._triggered_added:
                    self.convert_to_trigger()
                ...
                if percentage:
                    ...
                    adjustment = -stop_size
                    profit_price = self._calculate_new_price(priec=price, adjustment=adjustment, percentage=False)
                take_profit_order = {
                    "orderType": "STOP_LIMIT",
                    "session": "NORMAL",
                    "duration": "DAY",
                ...
                self.take_profit_order = stop_limit_order
    ...
    def add_leg(self, order_leg: int, symbol: str, quantity: int, asset_type: str, sub_asset_type: str= None) -> List[Dict]:
        # Define the leg
        leg={}
